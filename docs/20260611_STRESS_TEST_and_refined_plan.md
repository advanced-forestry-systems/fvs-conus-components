...
## 4j. BENCHMARK CLEAN, first trustworthy verdict (brk_dbh fix)

The tree-drop was the FVS small-tree breakpoint. With brk_dbh set to 99 (no breakpoint), FVS
retains every tree and the year-0 stand reproduces observed t1 exactly (BA and TPH bias 0.0 percent
for all models). All earlier absolute numbers were corrupted by the dropped large trees; this is the
first trustworthy verdict, on 20 remeasured NE plots:

| model | BA %RMSE | BA bias | TPH %RMSE | TPH bias | QMD %RMSE | QMD bias |
|---|---:|---:|---:|---:|---:|---:|
| real NE | 22.6 | +13.2 | 27.7 | -0.4 | 9.4 | +3.0 |
| real ACD | 21.4 | +12.2 | 29.8 | -4.9 | 10.0 | +4.3 |
| unified | 18.7 | **-0.6** | 59.4 | **-26.5** | 22.0 | +12.7 |

The picture is now real and nuanced, and it is the opposite of the corrupted earlier reading on the
key quantity. On **basal area**, the unified model is the best of the three: nearly unbiased (-0.6
percent) and lowest RMSE, while both real variants over-predict basal area by 12 to 13 percent. On
**density**, the unified model over-thins (-26.5 percent), which is the SDIMAX-too-low effect
isolated earlier; the real variants are nearly unbiased on stems. QMD inflates for the unified model
(+12.7 percent) as a direct consequence of the over-thinning (fewer, larger surviving trees).

So the honest verdict, on a clean benchmark, is that the unified model already beats the regional
variants on basal area (the quantity that drives volume and carbon) and loses on density purely
because of the SDIMAX over-thinning. That makes the SDIMAX recalibration not a detail but the single
change most likely to make the unified model win across the board: fix it and the density and QMD
should fall into line while the basal-area advantage holds. The benchmark is now a trustworthy
scoreboard, and the path to a winning variant is concrete and short on this evidence.
